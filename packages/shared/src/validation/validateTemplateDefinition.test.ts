...
describe("validateTemplateDefinition", () => {
  ...
  it("rejects an unknown slot type and a bad lock", () => {
    expect(
      validateTemplateDefinition(
        { ...valid, slots: [{ kind: "fixed", type: "ghost", lock: "open" }] },
        ctx,
      ).valid,
    ).toBe(false);
    expect(
      validateTemplateDefinition(
        { ...valid, slots: [{ kind: "fixed", type: "text", lock: "weird" }] },
        ctx,
      ).valid,
    ).toBe(false);
  });
  ...
  it("rejects a choice slot (not authorable in v1)", () => {
    expect(
      validateTemplateDefinition(
        {
          ...valid,
          slots: [{ kind: "choice", allowed: ["text"], default: "text", lock: "choice" }],
        },
        ctx,
      ).valid,
    ).toBe(false);
  ...
  it("rejects fixed data referencing a non-text or unknown field", () => {
    expect(
      validateTemplateDefinition(
        { ...valid, slots: [{ kind: "fixed", type: "text", lock: "fixed", data: { nope: "x" } }] },
        ctx,
      ).valid,
    ).toBe(false);
  });
