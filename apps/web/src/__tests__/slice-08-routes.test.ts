...
describe("proposals API — autosave round-trip (§13.8)", () => {
  it("create → list → get → autosave (PUT) → reload restores → delete", async () => {
    ...
    await saveProposal(json(`http://x/api/proposals/${created.id}`, "PUT", { ...sampleProposal, title: "Autosaved" }), ctx(created.id));

    const reloaded = (await (await getProposal(json(`http://x/api/proposals/${created.id}`, "GET"), ctx(created.id))).json()) as {
      proposal: { document: { title: string } };
    };
    expect(reloaded.proposal.document.title).toBe("Autosaved");

    const del = await deleteProposal(json(`http://x/api/proposals/${created.id}`, "DELETE"), ctx(created.id));
    expect(del.status).toBe(204);
  });

  it("404s on autosave/get for an unknown id", async () => {
    expect((await getProposal(json("http://x/api/proposals/nope", "GET"), ctx("nope"))).status).toBe(404);
    expect((await saveProposal(json("http://x/api/proposals/nope", "PUT", sampleProposal), ctx("nope"))).status).toBe(404);
  });

  it("400s on a malformed create body", async () => {
    expect((await createProposal(json("http://x/api/proposals", "POST", { nope: true }))).status).toBe(400);
  });
});
...
describe("versions API — export snapshots", () => {
  it("captures and lists a version", async () => {
    const created = await createOne();
    const snap = await snapshot(json(`http://x/api/proposals/${created.id}/versions`, "POST"), ctx(created.id));
    expect(snap.status).toBe(201);
    const list = (await (await listVersions(json("http://x", "GET"), ctx(created.id))).json()) as { versions: unknown[] };
