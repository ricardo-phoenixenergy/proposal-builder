...
describe("client/users", () => {
  ...
  it("createUser posts and returns the new summary", async () => {
    ...
    expect(await createUser({ email: "a@x.test", password: "longenough", isAdmin: false })).toEqual(
      summary,
    );
  ...
  it("throws the server error message on failure", async () => {
    ...
    await expect(
      createUser({ email: "a@x.test", password: "longenough", isAdmin: false }),
    ).rejects.toThrow("dup");
  });
